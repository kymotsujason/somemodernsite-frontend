...
class BlogView extends Component {
        ...
        Object.keys(data).map(index =>
            blog.push(
                ...
                >
                    ...
                    >
                        <div
                            className="remove_space spacing-half"
                            style={{
                                fontSize: this.state.height / 17,
                                fontWeight: "bold"
                            }}
                        >
                            {data[size - index].title}
                        </div>
                        <div
                            className="remove_space"
                            style={{ fontSize: this.state.height / 25 }}
                        >
                            by: Jason -{" "}
                            {new Intl.DateTimeFormat("en-CA", {
                                ...
                                new Date(data[size - index].published_date)
                            )}
                        </div>
                        <div
                            className="spacing-half linebreak"
                            style={{
                                height: this.state.actualHeight,
                                overflow: "hidden",
                                fontSize: this.state.height / 22
                            }}
                        >
                            {Parser(data[size - index].text)}
                        </div>
                        <br />
    ...
    render() {
        ...
        let height =
            window.innerHeight -
            document.getElementById("navbar").clientHeight * 2;
                                ...
                                <Card className="spacing">
                                    ...
                                    <div
                                        ...
                                        style={{
                                            height: height / 1.35,
                                            overflow: "auto",
                                            fontSize: (height - 300) / 32
                                        }}
                                    >
                                        {Parser(this.state.data.text)}
                                    </div>
                                </Card>
                            </div>
